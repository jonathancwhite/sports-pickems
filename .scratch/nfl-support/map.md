...
- [Seed the NFL classification and 2026 season](issues/05-seed-nfl-classification-and-season.md) — done in `d0c31b7`; `seedCatalog` now drives a two-row classification table (`ncaa-fbs`, `nfl`, both `tier: core`) and upserts a 2026 `upcoming` season for each, and the dev seed loops both classifications for weeks 1–3. Live: 48 NFL games seeded, all 8 division slugs on both sides of every one, FBS untouched at 902; both seeds re-run to a byte-identical snapshot. Found and fixed two pre-existing bugs — the dev seed never wrote `homeGroup`/`awayGroup`, and `pnpm db:seed:catalog` pointed at a module that called nothing.

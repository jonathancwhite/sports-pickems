...
- **`dates`, not `year`, selects the season.** `year` is accepted but inert — ESPN serves the current season whatever you pass, for both leagues. `dates=<season year>` works on both scoreboards and composes with `week` and `seasontype` (a `seasontype=3` request under `dates=2025` returns January-2026 playoff games as season 2025). Fixed in [ticket 10](issues/10-espn-year-param-ignored.md); the adapter now sends `dates` and throws on a returned/requested season mismatch. Live numbers reported by tickets 02 and 03 were the 2026 slate, not the season they named — the code paths they verified still hold, only the labels were wrong. Related: the FBS calendar is **per-season** (16 regular-season weeks in 2024 and 2025, 15 in 2026), so `regularSeasonWeekFallback: 15` is a 2026-shaped guess.
...
- [ESPN ignores the year param](issues/10-espn-year-param-ignored.md) — done in `dafa43c`; both fetchers send `dates` instead of `year`, and `fetchScoreboard`/`fetchRegularSeasonWeeks` now throw `EspnSeasonMismatchError` when ESPN's `season.year` disagrees with the season asked for, rather than upserting another season's games under it. The check is exact but skips when `season.year` is absent, so a trimmed payload can't take today's sync down. Site-API-wide, so it stays in `scoreboard.ts` and `LeagueConfig` is untouched. Live: real 2024/2025/2026 slates for both leagues, and NFL 1999 week 3. `games.ts` is unchanged — 04 should note the new throw.
